...
CARDS = "J23456789TQKA"  # leftmost: weakest, rightmost: strongest
CARDS_WITHOUT_JOKER = CARDS[1:]
...
def find_best_type(hand_original: str) -> HandType:
    if "J" not in hand_original:
        return get_type(hand_original)
    # else
    d: dict[HandType, list[str]] = {}
    for c in CARDS_WITHOUT_JOKER:
        hand = hand_original.replace("J", c)
        _type = get_type(hand)
        if _type not in d:
            d[_type] = []
        #
    ...
    #
    order = [
        HandType.FIVE_OF_A_KIND,  # best
        HandType.FOUR_OF_A_KIND,
        HandType.FULL_HOUSE,
        HandType.THREE_OF_A_KIND,
        HandType.TWO_PAIR,
        HandType.ONE_PAIR,
        HandType.HIGH_CARD,  # worst
    ]
    for key in order:
        if d.get(key):
            return key
...
class Game:
    def __init__(self, lines: list[str]) -> None:
        ...
        self.buckets: dict[HandType, list[str]] = {}  # will be set later
        #
        self.parse(lines)
    ...
    def sort_buckets(self) -> None:
        for li in self.buckets.values():
            # print("# before:", li)
            li.sort(key=functools.cmp_to_key(my_cmp))
            # print("# after:", li)
            # print("---")
